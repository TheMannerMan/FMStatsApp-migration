---
description: Create a feature spec file and branch from a short idea
argument-hint: Short feature description
allowed-tools: Read, Write, Glob, Bash(git switch:*), Bash(git branch:*), Bash(git status:*), Bash(git diff:*)
---

You are helping to spin up a new feature spec for this application, from a short idea provided in the user input below. Always adhere to any rules or requirements set out in any CLAUDE.md files when responding.

User input: $ARGUMENTS

## High level behavior

Your job will be to turn the user input above into:

- A human friendly feature title in kebab-case (e.g. new-heist-form)
- A safe git branch name not already taken (e.g. claude/feature/new-heist-form)
- A detailed markdown spec file under the _specs/ directory

Then save the spec file to disk and print a short summary of what you did.

## Step 1. Check the current branch

Check the current Git branch, and abort this entire process if there are any uncommitted, unstaged, or untracked files in the working directory. Tell the user to commit or stash changes before proceeding, and DO NOT GO ANY FURTHER.

## Step 1.5. Scan the codebase for context

Before drafting the spec, read through the project structure to understand existing naming conventions, architectural patterns, and relevant modules. Focus on files most likely related to the feature described in $ARGUMENTS. Use this context to inform the spec content in step 4.

Also verify that `_specs/template.md` exists. If it does not, abort and tell the user to create a template file before using this command.

## Step 2. Parse the arguments

From `$ARGUMENTS`, extract:

1. `feature_title`  
   - A short, human readable title in Title Case.  
   - Example: "Card Component for Dashboard Stats".

2. `feature_slug`  
   - A git safe slug.  
   - Rules:  
     - Lowercase 
     - Kebab-case 
     - Only `a-z`, `0-9` and `-`  
     - Replace spaces and punctuation with `-`  
     - Collapse multiple `-` into one  
     - Trim `-` from start and end  
     - Maximum length 40 characters  
   - Example: `card-component` or `card-component-dashboard`.

3. `branch_name`  
   - Format: `claude/feature/<feature_slug>`  
   - Example: `claude/feature/card-component`.

If you cannot infer a sensible `feature_title` and `feature_slug`, ask the user to clarify instead of guessing.

## Step 3. Switch to a new Git branch

Before making any content, switch to a new Git branch using the `branch_name` derived from the `$ARGUMENTS`. If the branch name is already taken, then append a version number to it: e.g. `claude/feature/card-component-01`

## Step 4. Draft the spec content

Use the `writing-plans` skill to structure the spec content appropriately for Plan mode consumption.

Create a markdown spec document that Plan mode can use directly and save it in the _specs folder using the `feature_slug`. Use the exact structure as defined in the spec template file here: @_specs/template.md.

Guidelines for filling in the template:
- Replace `<feature-name>` in the heading and branch field with `feature_slug`
- Write a concise **Summary** of what the feature does and why
- List **Functional Requirements** as clear, testable bullets derived from $ARGUMENTS and codebase context
- Only include the **Figma Design Reference** section if a Figma link or design is referenced in $ARGUMENTS — otherwise omit it entirely
- List realistic **Possible Edge Cases** based on the codebase context gathered in step 1.5
- Write **Acceptance Criteria** as specific, verifiable conditions
- Leave **Open Questions** for anything ambiguous that needs human input before implementation
- Fill in **Testing Guidelines** with meaningful but lightweight test cases relevant to the feature
- Do not add technical implementation details such as code examples
- Do not add a `created` frontmatter field or any other fields not present in the template

## Step 5. Verify and report

Use the `verification-before-completion` skill to confirm:
- The spec file exists on disk at the expected path
- The git branch has been created and is currently active
- No uncommitted changes exist beyond the new spec file

After verification, respond to the user with a short summary in this exact format:

Branch: <branch_name>
Spec file: _specs/<feature_slug>.md
Title: <feature_title>

Do not repeat the full spec in the chat output unless the user explicitly asks to see it. The main goal is to save the spec file and report where it lives and what branch name to use.